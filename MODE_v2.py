...
polyx, polyy = [], []
plt.axis([0, 10, 0, 60])
line10 = ax2.plot(polyx, polyy, 'ro' ,picker = 3)
plt.ylabel(r'$K_C$', fontsize='large')
...
#    good = plt.plot(por, tr, 'wo', label = "Stable" )
    ...
    line3 = ax1.plot(ti1[goodpoints], kc1[goodpoints], 'wo',
                     picker=5, label = "stable")  # adds good points
    line4 = ax1.plot(ti1[idx], kc1[idx], 'go', label = "Pareto Point")  # from pareto.data
    line5 = ax1.plot(
        gen_ti_unstable, gen_kc_unstable, 'r+',label = "unstable")  # unstable points from Gen
...
def redraw_ax2():
    ...
    ax2.cla()
    plt.axis([0, 10, 0, 60])
    ax2.plot(gen_td_idx, gen_kc_idx, 'bo',picker = 3)
    line10, = ax2.plot(polyx, polyy, 'go',picker = 3)
...
def redraw_ax3():
    obj_redrawer(fig, ppor, ptr, pise, piae, pitae, xobj, yobj,
                 Mop_points, por, tr, ISE, IAE, ITAE, gen_ppor, gen_ptr, gen_pise, gen_piae, gen_pitae, gen_por, gen_tr, gen_ise, gen_iae, gen_itae)
...
#            if event.artist!= line10:
#                print event.artist!= line10
#                return
...
class kc_td_interact():  
        def __init__(self):
              self.l2 = 0
              self.selectedtc, = ax2.plot(1, 1,'o',ms = 13,alpha = 0.5,color = 'orange',visible= False)
              self.correspondtc, = ax1.plot(1, 1, 'o', ms=12, alpha=0.4,
...
#              p2 = event.ind[m2]
        ...
        def update(self):
              ...
              ax1.plot((np.array(ti)[goodpoints])[p2], (np.array(kc)[goodpoints])[p2],'o',ms = 13,alpha = 0.5,color = 'yellow')
              yt = Y[p2]
              ax4 = fig.add_subplot(212)
              ax4.cla()
              plt.ylabel('y')
...
class pareto_interact():
    ...
    def onpick(self,event):
        print ax3.contains(event.mouseevent)[0]
...
class Point_generation():
    ...
    def plotter(self, kc,ti,td,x,num,entries,t,tfinal,dt,SP):
        ...
        global gen_ppor, gen_ptr, gen_pise, gen_piae, gen_pitae
        num =  np.shape(x)[0]
    ...
    #   calculates the risetime
        ...
        gen_ppor, gen_ptr, gen_pise, gen_piae, gen_pitae = ppor, ptr, pise, piae, pitae
    ...
    def generate(self,num_of_gens):
         ...
         if gen_count == [0]:
            ...
            for k in range(0, num):
                ...
                elif k == 1:        # Inserting Cohen-Coon parameters
                    kc = kcch
                    ti = tich
                    td = tdch
                else:
...
class System_setup(tk.Toplevel):
...
def Point_gen(self):
    Point_generation().generate(num_of_gens)
...
=======
import platform
>>>>>>> programs shows corresponding points
