...
from Sys_setup_window import System_setup
from Tuners import*
...
idx, ppor, ptr, pise, piae, pitae = [], [None], [None], [None], [None], [None]
...
#    good = plt.plot(por, tr, 'wo', label = "Stable" )
    ...
    line4 = ax1.plot(ti1[idx], kc1[idx], 'go')  # from pareto.data
    line5 = ax1.plot(
...
class Parasort:
    ...
    def Data_distillation(self, Y, kc, por, ti, tpr, tr, ISE, IAE, ITAE):
        ...
        SSoffset[0] = np.isneginf(por)
        UNSTABLE[0] = np.isnan(por)

        # good points are selected
        goodpoints[0] = ~(np.isnan(tr) | np.isnan(
...
#    GP_n, Gp_d, tfinal[0], dt, DT, SP, step_time, ramp_time, SP_input = sys_setup(
#    )
    wind = System_setup()
    wind.mainloop()
